    ...
    public static double ComputeTotalChance(this StatusEffectApplicationContext statusEffect)
    {
        return statusEffect.ChanceType switch
        {
            ...
            ChanceType.Base => statusEffect.ApplicationChance +
                               (statusEffect.Source.GetStat(nameof(Stat.EffectHitRate), statusEffect.Target as IBattleMember)) -
                               ((statusEffect.Target as IBattleMember)?.GetStat(nameof(Stat.EffectRes), statusEffect.Source) ?? 0),
